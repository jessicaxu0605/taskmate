...
function DayBoard({ slotsAvail, fillSlots, clearSlots, date, id }:DayBoardProps) {

    //frequency array: index corresponds to slotID, value of 0: slot is open; 1: slot is filled
    const filledTimeSlots =React.useRef<number[]>(Array.from({ length: 96 }, () => 0));
    ...
    function handleDrop(e: React.DragEvent<HTMLDivElement>) {
        ...
        if (slot && slotsAvail(id, slotNum, slotsRequired)) {
            ...
            if (dragOriginSlot instanceof HTMLElement) {
                ...
                if (IDParts[0].startsWith("DayBoard")) {              //check if the parent element is a slot, or it card is unscheduled
    ...
    function renderSlots() {
        ...
        for (let i = 0; i < 96; i++) {
            slots.push(
                <div id={`${id}slot${i}`} key={i} style={{height:`${timeSlotHeight}px`}} className={`${i % 4 == 0 ? 'border-t' : null} border-x`}></div>
            )
        }
        return slots;
    }
    ...
    return(
        ...
        >
            <div className={`grid grid-rows-96`}>
                {renderSlots()}
            </div>
        </div>
    )
...
export default function WeeklyView() {
    ...
    function renderTimeLabels() {
        ...
        for (let i = 1; i <= 12; i++) {
            timeLabels.push(<div key={`t${i}`} style={{height:`${4*timeSlotHeight}px`}}>{`${i}:00 AM`}</div>)
        }
        for (let i = 1; i <= 12; i++) {
            timeLabels.push(<div key={`t${i+12}`} style={{height:`${4*timeSlotHeight}px`}}>{`${i}:00 PM`}</div>)
        }
        return timeLabels;
    }
    ...
    return (
        <div style={{height:"500px"}} className={`grid grid-cols-8 border-slate-200 overflow-scroll h-1/2 relative`}>
            <div className='text-right pr-2'>{renderTimeLabels()}</div>
            {weekDays.map((val, index)=> 
            <div key={index}>
                <DayHeader date={val}/>
                <DayBoard slotsAvail={slotsAvail} fillSlots={fillSlots} clearSlots={clearSlots} id={`DayBoard${index}`} date={val} />
            </div>)}
        </div>   
    )
} 
