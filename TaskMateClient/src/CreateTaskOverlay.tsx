...
type inputErrors = "error: time needed to complete task cannot be 0" | "error: time needed to complete task cannot be more than 24 hours" | null

export default function CreateTaskOverlay({  closeOverlay  }: CreateTaskOverlayProps) {
    const [formInputs, setFormInputs] = React.useState<FormInputs>({name: null, dueDate: null, dueTime: null, durationHour: '0', durationMinute: '00'});
    ...
    function handleSubmit(e: React.FormEvent) {
        ...
        const reqBody = {
            ...
            duration: (formInputs.durationHour + ":" + formInputs.durationMinute + ":00")
        }
        console.log(reqBody);
        ...
        <div id='background' className='flex justify-center items-center w-screen h-screen top-0 right-0 fixed z-30 bg-slate-800 bg-opacity-80'
            ...
            <div style={{width:'40rem'}} className='bg-slate-300 br-10 text-left rounded-lg border-slate-400 border-2 opacity-100'>
                ...
                <form onSubmit={handleSubmit} className='px-16 py-10'>
                    ...
                    {inputError == null ? null : <div><h2>{inputError}</h2></div>}
                    <div className='flex flex-row justify-center m-4'>            
                        <input type='submit' value='CREATE TASK' className='bg-red-800 rounded-full text-white font-bold py-3 px-6'/>
                    </div>
                </form>
            </div>
        </div>
